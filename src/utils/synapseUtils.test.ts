...
test('getDominantSynapseType: handles tie by returning one of the tied types (first wins)', () => {
  const typeCounts: Record<SynapseType, number> = {
    minor: 10,
    complex: 10,
    deep: 5,
  ...
  const result = getDominantSynapseType(typeCounts)
  // Should return one of the tied types (implementation-dependent)
  assert(
...
test('Edge case: Cluster with all types at equal count', () => {
  ...
  const dominantType = getDominantSynapseType(cluster.typeCounts)
  // Should return one of the types (implementation-dependent due to Object.entries order)
...
test('Config check: Level progression is correct', () => {
  // Verify that higher rarity types require higher or equal levels
  ...
  // Verify level progression: rare >= core, legendary >= rare, unique >= legendary
