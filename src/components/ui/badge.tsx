  ...
  'inline-flex items-center gap-1 rounded-full font-semibold transition-all duration-200',
  {
    variants: {
      variant: {
        ...
        exploring: 'bg-[hsl(var(--tier-team))]/20 text-[hsl(var(--tier-team))] border border-[hsl(var(--tier-team))]/30',
...
interface StatusBadgeProps extends Omit<BadgeProps, 'children' | 'variant'> {
  status: 'online' | 'offline' | 'busy' | 'away' | 'active' | 'idle' | 'solving' | 'exploring' | 'resting'
  label?: string
  showDot?: boolean
}
...
const statusConfig = {
  ...
  exploring: { variant: 'exploring' as const, dotColor: 'blue' as const, label: 'Exploring' },
